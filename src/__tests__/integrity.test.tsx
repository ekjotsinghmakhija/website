...
describe("Project & Blog Integrity", () => {
  it("verifies that every blog registry entry has a physical file", () => {
    BLOG_POSTS.forEach((post) => {
      ...
      if (!fs.existsSync(pagePath)) {
        throw new Error(
          `CRITICAL: Registry slug "${post.slug}" missing file at ${pagePath}`,
        );
      }
    });
  });
  ...
  it("verifies that all featured projects have a corresponding blog post", () => {
    ...
    PERSONAL_PROJECTS.filter((p) => p.featured).forEach((project) => {
      const found = blogSlugs.some(
        (slug) => slug.includes(project.id) || project.id.includes(slug),
      );
      if (!found) {
        throw new Error(
          `WARNING: Featured project "${project.title}" (${project.id}) has no matching blog entry.`,
        );
      }
    });
  });
    ...
    physicalItems.forEach((item) => {
      ...
      if (fs.statSync(itemPath).isDirectory()) {
        if (!validSlugs.has(item)) {
          throw new Error(
            `CRITICAL: Rogue folder found! "${item}" is not a registered blog slug in src/lib/blog.ts. Please delete this folder or fix its casing.`,
          );
        }
      }
    });
